...
class BasePipeline(torch.nn.Module):
    ...
    def generate_noise_like(self, input, seed=None, device="cpu", dtype=torch.float16):
        generator = None if seed is None else torch.Generator(device).manual_seed(seed)
        noise = torch.randn(input.size(), generator=generator, device=device, dtype=dtype, layout=input.layout)
        return noise
